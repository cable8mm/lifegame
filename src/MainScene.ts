...
export default class MainScene extends Phaser.Scene {
  ...
  public preload(): void {
    this.graphics = this.add.graphics({
      fillStyle: { color: GameConfig.CELL_COLOR },
    });
  }
  ...
  public create(): void {
    ...
    Array.from(Array(this.stageSize).keys()).forEach((index) => {
      this.cells.push(
        new Cell(
          this,
          this.numberToPosition(index),
          GameConfig.CELL_SEED > Math.random()
        )
      );
    });
  ...
  private nextGeneration(): void {
    this.generation++;
    Array.from(Array(this.stageSize).keys()).forEach((index) => {
      this.cells[index].score = this.neighborScore(index);
    });
  }
  ...
  private neighborScore(index: number): number {
    ...
    if (
      ...
      neighbor.left &&
      this.cells[
        this.positionToNumber({ x: position.x - 1, y: position.y - 1 })
      ]?.active
    )
      score++;
    if (
      neighbor.up &&
      this.cells[this.positionToNumber({ x: position.x, y: position.y - 1 })]
        ?.active
    )
      score++;
    if (
    ...
    if (
      neighbor.left &&
      this.cells[this.positionToNumber({ x: position.x - 1, y: position.y })]
        ?.active
    )
      score++;
    if (
